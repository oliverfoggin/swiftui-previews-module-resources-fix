...
struct BundleCurrentPlugin: BuildToolPlugin {
  func createBuildCommands(context: PluginContext, target: Target) async throws -> [Command] {
    ...
    guard let sourceModuleTarget = target as? SwiftSourceModuleTarget,
      sourceModuleTarget.hasResources
    else {
      return []
    }
    ...
    let content = generateBundleExtensionSwift(
      packageName: packageName,
      targetName: targetName)

    do {
      try FileManager.default.createDirectory(
        at: URL(fileURLWithPath: outputDir.string),
        withIntermediateDirectories: true)

      try content.write(
        toFile: outputFilePath.string,
        atomically: true,
        encoding: .utf8)
